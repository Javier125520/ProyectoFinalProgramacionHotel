...
public class ReservaServicioDAO {
    private final static String SQL_INSERT = "INSERT INTO reserva_servicio (fechaReserva, precio, fechaInicio, fechaFin) VALUES (?, ?, ?, ?)";
    ...
    public static void insertReservaServicio(ReservaServicio reserva) {
        try (PreparedStatement pst = ConnectionBD.getConnection().prepareStatement(SQL_INSERT)) {
            ...
            pst.setInt(2, reserva.getPrecio());
            pst.setDate(3, Date.valueOf(reserva.getFechaInicio()));
            pst.setDate(4, Date.valueOf(reserva.getFechaFin()));
            pst.executeUpdate();
